...
flowchart TD
    subgraph 메타데이터
        A1["날짜<br>계량일자 / 날짜 / 일시 / 일자"]
        A2["차량번호<br>차량번호 / 차번호 / 차량No."]
    end

    subgraph 거래정보
        B1["거래처<br>거래처 / 상호 / 귀하 패턴"]
        B2["발급회사<br>(주) / 주식회사 / 하단 휴리스틱"]
        B3["회사주소<br>광역시·도 패턴 매칭"]
    end
...
`requirements.txt`에 spaCy가 포함되어 있으나, 현재 파싱 로직에서는 사용하지 않습니다.

| 비교 항목 | 정규식 (현재) | spaCy NER |
|---|---|---|
| 방식 | 키워드 매칭 + 정규식 | 텍스트를 통째로 넣으면 ORG/LOC/DATE 등 자동 분류 |
| OCR 오탈자 대응 | 직접 정규화 로직으로 제어 가능 | 한국어 모델이 오탈자에 취약 (예: "곰욕환경폐기물") |
| 도메인 특화 값 | `13,460 kg` 등 정규식으로 정확히 추출 | 무게값은 결국 정규식 필요 |
| 오버헤드 | 없음 | 모델 로딩 수백MB |
| 적합한 시점 | 양식이 정형화된 현재 상황 | 양식이 다양해지거나 라벨 자체가 없는 문서 처리 시 |

### 개선 아이디어

- **오탈자 교정**: 하드코딩 사전 대신 편집 거리(Levenshtein distance) 기반 유사도 매칭으로 확장
- **NER 활용**: 계근지 양식이 다양해질 경우 spaCy NER로 라벨 없이도 회사명/날짜/주소를 추출
