...
전처리 단계(cleaner)는 도메인 지식을 반영한 최소한의 정규화/교정을 수행합니다.

- 표제 정규화: `계 그 표` → `계근표`
  - 배경: sample_02의 표제이며, 현장 표준 용어는 “계근표”입니다. OCR에서 ‘근’이 ‘그’로 오인식된 사례를 보수적으로 교정합니다.
- 업체명 OCR 교정: `곰욕환경폐기물` → `고요환경`
  - 배경: sample_01의 업체명 오인식이며, 동일 차량번호(8713)가 등장하는 sample_02에서 “고요환경”이 확인됩니다.
  - 운영 권고: 실제 운영에서는 차량번호·기간 기반의 마스터데이터로 교차검증하는 것이 바람직합니다. 본 리포지토리에서는 예시 수준의 보수적 치환만 반영했습니다.
